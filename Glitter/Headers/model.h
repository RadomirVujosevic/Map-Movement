   ...
   private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene) {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
            Vertex vertex;
            glm::vec3
                vector;  // we declare a placeholder vector since assimp uses
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
            ...
            for (unsigned int j = 0; j < textures_loaded.size(); j++) {
                if (std::strcmp(textures_loaded[j].path.data(), str.C_Str()) ==
                    ...
                    skip = true;  // a texture with the same filepath has
                                  // already been loaded, continue to next one.
                                  // (optimization)
                    break;
                }
            }
            if (!skip) {  // if texture hasn't been loaded already, load it
                ...
                textures_loaded.push_back(
                    texture);  // store it as texture loaded for entire model,
                               // to ensure we won't unnecesery load duplicate
                               // textures.
            }
        }
        return textures;
